package code {
	...
	/** this is the SceneWin class, and it is extening(giving its info) to the GameScene class so that it can use its info */
	public class SceneWin extends GameScene {
		...
		override public function update(): GameScene {
			...
			if (shouldSwitchToPlay) return new SceneTitle(); // if enter is pressed on the keyboard, change back to the title screen
